      ...
      // val tt= data_std.map(x=> x.toArray).map(DenseVector(_).toVector)
        ...
        if (initialized) {
         /* if (this.removeSW) {
          ...
          val assignations = assignToMicroCluster(processedData)
          updateMicroClusters(assignations)
         ...
         /* durationStep += Duration.between(t0, t1).toMillis
          ...
          val t00=Instant.now
          minInitPoints match {
            case 0 => initRand(processedData)
            case _ => if (useNormalKMeans) initKmeans(processedData) else initStreamingKmeans(processedData)
          }
           val t11=Instant.now
